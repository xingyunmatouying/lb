...
from src.leaderboard.data.leaderboard_row import BotInfo, BotProfile, LeaderboardPerf, LeaderboardRow, RankInfo
...
BOT_2_ROW = LeaderboardRow(BOT_2_INFO, RankInfo(1, 0, 0, 1, 1800, False, True))
...
class TestLeaderboardUpdate(unittest.TestCase):
  ...
  def test_row_only_update(self) -> None:
    ...
    previous_row = LeaderboardRow(previous_bot_info, RankInfo(5, 0, 0, 3, 1500, True, True))
    ...
    self.assertEqual(update.to_leaderboard_row(2), LeaderboardRow(previous_bot_info, RankInfo(2, 3, 0, 2, 1500, False, False)))
  ...
  def test_bot_info_only_update(self) -> None:
    ...
    self.assertEqual(update.get_rating(), 1500)
    self.assertEqual(update.get_created_time(), DATE_2024_01_01)
    self.assertEqual(update.to_leaderboard_row(2), LeaderboardRow(previous_bot_info, RankInfo(2, 0, 0, 2, 1500, True, True)))
  ...
  def test_full_update(self) -> None:
    ...
    self.assertEqual(update.get_created_time(), DATE_2024_01_01)
    self.assertEqual(update.to_leaderboard_row(1), LeaderboardRow(current_bot_info, RankInfo(1, 4, 100, 1, 1600, False, True)))
